...
[ ] Provider Chain (多模型责任链)
  - [ ] 引入 `ProviderChain` 责任链，按配置顺序尝试多个模型/提供商，失败则切到下一个
  - [ ] 实现 `DefaultProvider` 作为末节点兜底，返回“系统繁忙”信息（策略可配）
...
- [ ] 责任链顺序由 `application.yml` 决定，节点启停/顺序变更可通过配置切换
- [ ] 上游全部失败时，由 `DefaultProvider` 返回“系统繁忙”或按策略回退（记录明确的链路与原因）
